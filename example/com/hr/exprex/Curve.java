...
public class Curve {
	...
	protected double unscreenifyX(int xCoord) {
		return (xCoord - x0) / xScale;
	}

	protected double unscreenifyY(int yCoord) {
		return (y0 - yCoord) / yScale;
	}

	protected int screenifyX(double x) {
		return (int) (0.5 + x0 + x * xScale);
	}

	protected int screenifyY(double y) {
		return (int) (0.5 + y0 - y * yScale);
	}
}
